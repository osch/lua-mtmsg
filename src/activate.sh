# Activations script for add the mtmsg module to the lua path when
# building mtmsg using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if mtmsg is installed, e.g. via luarocks.

this_dir=$(pwd)

luamtmsg_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$luamtmsg_dir/src/activate.sh" -o ! -e "$luamtmsg_dir/src/main.c" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "examples" directory ***'

else

    echo "Setting lua paths for: $luamtmsg_dir"

    add_lua_path="$luamtmsg_dir/src/?.lua;$luamtmsg_dir/src/?/init.lua"
    add_lua_cpath="$luamtmsg_dir/src/build"
