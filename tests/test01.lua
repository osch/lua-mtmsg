...
do
    ...
    local ok, err = pcall(function() mtmsg.buffer("hugo") end)
    assert(err:match(mtmsg.error.ambiguous_name))
    b:addmsg("m1")
    b:close()
    ...
    print(err)
    print("---------------------")
    assert(err:match(mtmsg.error.object_closed))
end
collectgarbage()
do
    ...
    print(err)
    print("---------------------")
    assert(err:match(mtmsg.error.message_size))
...
for i = 1, 2 do
    ...
    print("------ expected error")
    print(err)
    assert(err:match(mtmsg.error.operation_aborted))
    print("---------------------")
